                ...
                if (args.length == 0) {
                    if (sender.hasPermission("fly.use")) {
                        if (sender instanceof Player) {
                            ...
                            sender.sendMessage("§4Questo comando possono eseguirlo solo i player prova /fly on <player> o /fly <player>");
                            return true;
                        }
                    } else {
                        ...
                        if (target != null) {
                            if (args[0].equalsIgnoreCase("on")) {
                                if (target.getAllowFlight()) {
                                    ...
                                    sender.sendMessage("§2fly messa a " + target.getName());
                                }
                            } else if (args[0].equalsIgnoreCase("off")) {
                                if (target.getAllowFlight()) {
                                    target.setAllowFlight(false);
                                    sender.sendMessage("§4Fly tolta a " + target.getName());
                                } else {
                    ...
                    if (sender.hasPermission("fly.admin")) {
                        Player target1 = Bukkit.getServer().getPlayerExact(args[0]);
                        if (target1 != null) {
                            if (target1.getAllowFlight()) {
                                target1.setAllowFlight(false);
                                sender.sendMessage("§4Fly tolta a " + target1.getName());
                            } else {
                                target1.setAllowFlight(true);
                                sender.sendMessage("§2Fly messa a " + target1.getName());
                            }
                        } else {
                            sender.sendMessage("§4Il player non è online!");
                        }
